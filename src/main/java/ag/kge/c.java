...
//c.k("0N!",NULL('z'));
//c.setEncoding("UTF-8");O("Unicode "+c.k("{`$x}","Ranby Bj????rklund AB".toCharArray()));O("Unicode "+c.k("{x}",(String)c.k("{`$x}",(char[])c.k("\"c\"$0x52616e627920426ac3b6726b6c756e64204142"))));   
...
public static void setEncoding(String e)throws UnsupportedEncodingException{
    c.e=e;out=new PrintStream(System.out,true,e);
}
public Socket s;
DataInputStream i;
OutputStream o;
byte[]b,B; int j,J,vt; boolean a;
void io(Socket x) throws IOException{
    s=x;i=new DataInputStream(s.getInputStream());o=s.getOutputStream();
}
public void close()throws IOException{if(null!=s){s.close();s=null;};if(null!=i){i.close();i=null;}if(null!=o){o.close();o=null;}}
